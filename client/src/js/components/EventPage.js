...
export class EventPage {
  ...
  createGoogleCalendarUrl(eventData)
  {
    ...
    let dateString;
    if (eventData.allDay) {
      // Google Calendar end date is exclusive for all-day events — add 1 day
      const d = eventData.endDate;
      const nd = new Date(+d.slice(0, 4), +d.slice(4, 6) - 1, +d.slice(6, 8) + 1);
      const exclusiveEnd = nd.getFullYear().toString()
        + String(nd.getMonth() + 1).padStart(2, '0')
        + String(nd.getDate()).padStart(2, '0');
      dateString = `${eventData.startDate}/${exclusiveEnd}`;
    } else {
